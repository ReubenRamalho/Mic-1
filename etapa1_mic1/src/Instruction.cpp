...
Instruction::Instruction() : f0(0), f1(0), ena(0), enb(0), inva(0), inc(0), rawBits("000000") {}

Instruction::Instruction(int f0, int f1, int ena, int enb, int inva, int inc, const std::string& rawBits)
    : f0(f0), f1(f1), ena(ena), enb(enb), inva(inva), inc(inc), rawBits(rawBits) {}

Instruction Instruction::fromString(const std::string& line) {
    if (line.size() != 6) {
        throw std::invalid_argument("Cada instrução deve conter exatamente 6 bits.");
    }
