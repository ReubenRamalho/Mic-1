...
Instruction::Instruction() : sll8(0), sra1(0), f0(0), f1(0), ena(0), enb(0), inva(0), inc(0), ulaBits("00000000"), cBits("000000000"), bBits("0000") {}

Instruction::Instruction(
    int sll8, int sra1, int f0, int f1, int ena, int enb, int inva, int inc,
    const std::string &ulaBits, const std::string &cBits, const std::string &bBits) : sll8(sll8), sra1(sra1), f0(f0), f1(f1), ena(ena), enb(enb), inva(inva), inc(inc), ulaBits(ulaBits), cBits(cBits), bBits(bBits) {}

Instruction Instruction::fromString(const std::string &line)
{
    if (line.size() != 21)
    {
        throw std::invalid_argument("Cada instrução deve conter exatamente 21 bits.");
    }
    ...
    std::string bBits = line.substr(17, 4);
