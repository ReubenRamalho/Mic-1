...
void ProgramMemory::loadFromFile(const std::string &fileName) {
  ...
  if (!file.is_open()) {
    throw std::runtime_error("Nao foi possivel abrir o arquivo de programa: " +
                             fileName);
  }
  ...
  while (std::getline(file, line)) {
    ...
    try {
      ...
      throw std::runtime_error("Erro na linha " + std::to_string(lineNumber) +
                               ": " + e.what());
    }
  }
}
 ...
 */
std::size_t ProgramMemory::size() const { return instructions.size(); }
...
void ProgramMemory::appendInstructions(
    const std::vector<Instruction> &newInstr) {
  instructions.insert(instructions.end(), newInstr.begin(), newInstr.end());
}